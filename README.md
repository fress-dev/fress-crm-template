...
> **テンプレートリポジトリ** — [marmelab/atomic-crm](https://github.com/marmelab/atomic-crm) を土台に、コアを変更せず `src/custom/` で拡張するためのベースです。

## リモート構成

| リモート | URL | 用途 |
|----------|-----|------|
| `origin` | `https://github.com/fress-dev/fress-crm-template.git` | このテンプレート（push 先） |
| `upstream` | `https://github.com/marmelab/atomic-crm.git` | 上流（fetch のみ推奨） |
